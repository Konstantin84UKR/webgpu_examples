...
const webGPU_Start = async () => {
    ...
    // PLANE 
    //****************** BUFFER  vertexBuffer
    const vertexBufferPlane = device.createBuffer({
      ...
      //загружаем данные в буффер */
      new Float32Array(vertexBufferPlane.getMappedRange()).set(plane_vertex);
      // передаем буфер в управление ГПУ */
      vertexBufferPlane.unmap();
  
      //****************** BUFFER  uvBuffer
      const uvBufferPlane = device.createBuffer({
        size: plane_uv.byteLength,
        usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST,   //COPY_DST  ХЗ что это
        mappedAtCreation: true
      });
      //загружаем данные в буффер */
      new Float32Array(uvBufferPlane.getMappedRange()).set(plane_uv);
      // передаем буфер в управление ГПУ */
      uvBufferPlane.unmap();
  
     //****************** BUFFER  normalBuffer
      const normalBufferPLane = device.createBuffer({
        size: plane_normal.byteLength,
        usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST,   //COPY_DST  ХЗ что это
        mappedAtCreation: true
      });
      //загружаем данные в буффер */
      new Float32Array(normalBufferPLane.getMappedRange()).set(plane_normal);
      // передаем буфер в управление ГПУ */
      normalBufferPLane.unmap();
  
     //****************** BUFFER  indexBuffer
      const indexBufferPlane = device.createBuffer({
        size: plane_index.byteLength,
        usage: GPUBufferUsage.INDEX | GPUBufferUsage.COPY_DST,
        mappedAtCreation: true
      });
  
      new Uint32Array(indexBufferPlane.getMappedRange()).set(plane_index);
      indexBufferPlane.unmap();  
 ...
 //*********************************************//
    ...
    function animate(time) {
        ...
        renderPass.setVertexBuffer(0, vertexBufferPlane);
        renderPass.setVertexBuffer(1, uvBufferPlane);
        renderPass.setVertexBuffer(2, normalBufferPLane);
        renderPass.setIndexBuffer(indexBufferPlane, "uint32");
        renderPass.setBindGroup(0, uniformBindGroup);
