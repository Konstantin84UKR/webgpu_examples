...
function loadFile(result,modelURL) {
  var xhr = new XMLHttpRequest();
  //var model;
...
async function main() {
    ...
    await loadFile(CUBE,'./res/africa.obj');
    let modelData = await loadOBJ(CUBE.mesh);

    //let mesh = CUBE.mesh.meshes[0];

     const cube_vertex = new Float32Array(modelData.vertex.flat());
     const cube_uv = new Float32Array(modelData.uv.flat());
     const cube_index = new Uint32Array(modelData.faces.flat());
    // const cube_index = new Uint32Array(modelData.faces.flat());
    //---------------------------------------------------
    ...
    img.src = './res/african.jpg'; //'./tex/african.jpg';
    await img.decode();
