...
export const computeShader = {
    compute:`
        ...
        let WorkGroupTEST = WorkGroupID.xy + vec2(WorkGroupID.x * 2, 16); 

        let baseIndex = vec2<i32>(WorkGroupTEST.xy * vec2(params.blockDim, 4) + LocalInvocationID.xy * vec2(4, 1)) - vec2(filterOffset, 0);

      
        //     1 поток      2 поток       3 поток          32 поток 
        //(LocalInvocationID)          X = baseIndex
        // r1 |0 0 0 0|    |0 0 0 0|     |X R 0 0|        |0 0 0 0|   всего 128 ячейки (WorkGroupID)
        ...
        for (var r = 0; r < 4; r++) {
            for (var c = 0; c < 4; c++) {
                ...
                //if (all(writeIndex < dims)) {    
                        ...
                        //textureStore(outputTex, writeIndex, vec4(1.0, 0.0, 0.0, 1.0) );
                       
                    }else if ( all(writeIndex < dims)) {
                        textureStore(outputTex, writeIndex, vec4(0.0, 0.0, 1.0, 1.0) );
                    }
