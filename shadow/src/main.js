...
// 3. Исправил матрицу нормалей убрал ветор перемешения.
...
// 5. Инвертировал ось Z в буффере глуны. Для этого изменил значение очистки буффера на 0.0
...
async function main() {
  ...
  const shadowGroupModel1 = device.createBindGroup({
    label: 'Group for shadowPass',
    //layout: shadowPipeline.getBindGroupLayout(0),
  ...
  const shadowGroupModel2 = device.createBindGroup({
    label: 'Group for shadowPass',
    //layout: shadowPipeline.getBindGroupLayout(0),
    layout: bindGroupLayout_1_shadowPipeline,
    entries: [{
  ...
  MODELMATRIX = mat4.translate(MODELMATRIX, vec3.set(0,1.8,0));

  device.queue.writeBuffer(uniformBufferModel, 0, MODELMATRIX); // и так дале прибавляем 64 к offset
  device.queue.writeBuffer(uniformBufferModel_2, 0, MODELMATRIX_PLANE); // и так дале прибавляем 64 к offset
  ...
  async function animate(time) {
    ...
    device.queue.writeBuffer(uniformBuffer, 0, reversZpMatrix); // пишем в начало буффера с отступом (offset = 0)
    device.queue.writeBuffer(uniformBuffer, 64, camera.vMatrix); // следуюшая записать в буфер с отступом (offset = 64)
    // device.queue.writeBuffer(uniformBuffer, 64 + 64, MODELMATRIX); // и так дале прибавляем 64 к offset
    // device.queue.writeBuffer(uniformBuffershadow, 64 + 64, MODELMATRIX); // и так дале прибавляем 64 к offset
    ...
    renderPassShadow.setBindGroup(1, shadowGroupModel1);
    ...
    renderPassShadow.setBindGroup(0, shadowGroup);
    renderPassShadow.setBindGroup(1, shadowGroupModel2);
    ...
    renderPass.setBindGroup(2, uniformBindGroup2);
    ...
    renderPass.setBindGroup(0, uniformBindGroup);
    renderPass.setBindGroup(1, uniformBindGroup1);
    renderPass.setBindGroup(2, uniformBindGroup2_1);
