...
import { shaderCubeMap } from './cubeMapPlane/shaderCubeMap.js';
...
import { initPipeline as initCubeMapPipeline} from './cubeMapPlane/initPipeline.js';
   ...
   //const radiuses = new Array(instance_count).fill(0).map(_ =>  2.0);
     ...
     let k = 0;
      ...
      const bodies = centers.map((center, i) => {
        ...
        const colliderDesc = RAPIER.ColliderDesc.ball(radiuses[i]+0.1).setFriction(0.5);
    ...
    const {pipeline: pipelineCubeMap } = await initCubeMapPipeline(device,format,shaderCubeMap,uBuffers,textureCUBE,sampleCount)
    ...
    device.queue.writeBuffer(uBuffers.uniformBufferInvMatrix, 0, mat4.inverse(INVERSE_PV_MATRIX, INVERSE_PV_MATRIX)); // и так дале прибавляем 64 к offset
...
window.addEventListener("keyup", () => sign = -2);
 ...
 async function animate(time) {
      ...
      const renderPass = commandEncoder.beginRenderPass(renderPassDescription);
       //Env
       renderPass.setPipeline(pipelineCubeMap);  
       renderPass.setBindGroup(0, pipelineCubeMap.BindGroup.uniformBindGroup); 
       renderPass.draw(6); 
